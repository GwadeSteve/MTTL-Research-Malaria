...
\begin{itemize}
	\item \textbf{Data Consistency:} The patient-stratified train, validation, and test splits used for MTTL were converted to YOLO format, guaranteeing evaluation on identical unseen data.
...
\begin{table}[htbp]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{l c c c c}
			...
			F1-Macro & 0.570 & 0.581 & \textbf{0.904} & +55.6\% \\
...
\noindent The improvement at the stricter mAP@75 threshold is notable: MTTL outperforms YOLOv8-Small by $18.6\%$, indicating tighter, more accurate bounding boxes. The macro-averaged F1-score also shows a substantial $55.6\%$ gain, reflecting balanced performance across all three classes rather than dominance by the majority class.
...
\noindent Clinically, this accuracy difference is vital. Since the WHO classifies malaria severity based on parasitemia thresholds, a $3.78\%$ error could lead to misclassification, whereas our $1.08\%$ error remains within acceptable diagnostic bounds. This reliability stems from the multi-task architecture: auxiliary segmentation and RoI classification tasks regularize learning, forcing the feature extractor to capture structural parasite representations that single-task detection objectives miss.\\

\noindent We conclude that, for minority class detection in imbalanced medical datasets, multi-task learning offers distinct advantages over highly optimized single-task detectors. While YOLOv8 is faster and more parameter-efficient, the diagnostic reliability gained from MTTL justifies the additional computational cost for clinical applications where accuracy on rare but critical classes is crucial.