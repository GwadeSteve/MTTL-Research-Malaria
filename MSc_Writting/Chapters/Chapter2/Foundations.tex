...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% To represent channels/depth
		\draw (input_data.north east) -- ++(0.2,0.2) -- ++(0,-1.2*0.85) -- (input_data.south east); % 0.85 is the node scale
		...
		\node[data_flow, right=0.2cm of flatten_op, minimum width=0.3cm, minimum height=1.5cm, label={[font=\tiny]below:Vector}] (vector_data) {}; % Representing a vector
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% Input Patch (e.g., 3x3 for simplicity)
		...
		% Highlight the region of the input patch covered by the filter (top-left 2x2)
		...
		% Label showing the filter "slides"
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[
		...
		% Highlight pooling windows (dashed only, no fill)
		...
		% Arrows from windows to output cells
		...
		% Stride arrows inside the input map
...
\begin{center} 
	...
	\begin{longtable}{@{} 
			...
			>{\centering\arraybackslash}p{0.05\textwidth}     % Year (very narrow)
			>{\raggedright\arraybackslash}p{0.28\textwidth}  % Task (combined with dataset info)
...
\noindent Beyond simple fine-tuning, researchers have expanded into ensemble strategies \parencite{Nayak2022EnsembleAI, Bibin2017MalariaDBN}, hybrid pipelines merging CNN features with classifiers like SVMs \parencite{LiMa2022ResidualAttentionSVM}, and object detection using YOLO variants \parencite{Abdurahman2021ModifiedYOLO, HoyosHoyos2024YOLOv8Malaria}. Given the scarcity of annotated samples, data augmentation is also standard practice. Despite these methodological advances, recurring limitations persists. Models ouputs are partial and not enough for a full diagnosis in clincal settings, also models that perform well on benchmarks often degrade when faced with variations in clinics, staining protocols, or imaging devices. Additionally, inconsistent reporting of experimental details, such as smear types and dataset curation, often complicates reproducibility and the assessment of true robustness. \\

\noindent
While transfer learning has built a strong foundation for automated classification, the focus has largely remained on the binary distinction between infected and uninfected cells. A complete clinical diagnosis usually requires broader insights, including species identification, life-stage classification, and parasitemia estimation. Since standard TL frameworks typically optimize for single-task objectives, there is a clear opportunity to explore methods that better align with the multi-faceted nature of malaria diagnosis.

\subsection{Multitask and Multitask Transfer Learning Relevance}
\label{ssec:mtl_mttl_malaria}
In contrast to single-task paradigms, multitask learning (\ac{MTL}) and its extension, multitask transfer learning (MTTL), provide a framework for jointly addressing multiple outputs that share visual structure. Tasks such as distinguishing \textit{Plasmodium} species, identifying parasite life stages, and estimating parasitemia density are interrelated. A shared backbone guided by pre-trained weights could capture these correlations more effectively than training separate models. Despite this promise, no systematic application of MTL or MTTL to malaria imaging has yet been reported.\\

\noindent
Outside the malaria domain, \ac{MTL} and \ac{MTTL} have shown substantial utility in medical imaging tasks. For example, \textcite{Yadav2024_MT_TL_ActiveMedSeg} explored multitask transfer learning in an active learning pipeline for medical image segmentation, demonstrating improved annotation efficiency. \textcite{Huang2022MTLABS3Net} employed a multi-task framework for organ segmentation and anatomical prior prediction, achieving robust semi-supervised learning performance. Similarly, \textcite{Kim2023CrossTaskAttention} introduced a cross-task attention mechanism to enhance multi-task learning for diverse medical imaging applications. \textcite{Chandani2023Investigating} and \textcite{Dheer2023Exploiting} investigated MTTL strategies for segmentation tasks, showing that shared feature representations improve accuracy across related tasks. \textcite{Schoenpflug2023_MTL_CRC} applied multitask learning to simultaneously segment tissues and detect tumors in colorectal cancer histology slides, while \textcite{Li2023_MTL_NSCLC} leveraged MTL to classify histologic subtypes and tumor grade in non-small cell lung cancer CT scans. Even in robotic surgery, \textcite{Islam2021StMTLSpatioTemporal} developed a spatio-temporal MTL model to jointly predict instrument tracking paths and surgeon scanpaths. Beyond medical vision, MTTL has also been applied to non-medical domains such as natural language processing and multi-view autonomous driving perception \parencite{Zhang2019_MultiviewTL_MTL, raffel2020exploring}, demonstrating both efficiency and robustness benefits.\\

\noindent
These studies collectively illustrate that multitask and multitask transfer paradigms can capture shared structure across related tasks, improving generalization and reducing the need for separate models an opportunity that remains largely unexplored in malaria diagnosis.

\subsection{Observed Gaps in the Literature}
\label{ssec:literature_gaps}
While research has advanced, significant gaps remain that limit clinical utility. These issues become clear when viewing malaria diagnosis as a complex, multi-step process rather than a simple classification task. Key limitations include:

\begin{itemize}
	\item \textbf{Focus on binary outputs:} Many studies concentrate primarily on distinguishing parasitized from uninfected cells. While useful, this binary output does not provide the complete diagnostic profile, such as species identification or parasitemia estimation, which is required for effective treatment decisions.
	\item \textbf{Absence of multitask frameworks:} To date, there has been no exploration of MTL or MTTL for malaria detection, despite how well these approaches align with the natural workflow of microscopic diagnosis.
	\item \textbf{Theoretical limitations:} Current literature lacks a formalized framework that adapts multitask transfer principles specifically for malaria or similar diagnostic challenges.
	\item \textbf{Reporting inconsistencies:} Reproducibility is often difficult because studies frequently omit key details regarding smear types, staining protocols, or preprocessing steps.
	\item \textbf{Dataset constraints:} Public datasets are often small and homogeneous, which limits how well models generalize across different clinics or imaging equipment.\\
\end{itemize}

\noindent
Prior research highlights the potential of transfer learning but often lacks a holistic approach. The limited scope of existing studies, combined with the absence of multitask strategies, points to a clear need for more comprehensive methods. This gap motivates our focus on developing a multitask transfer learning framework for malaria diagnosis.