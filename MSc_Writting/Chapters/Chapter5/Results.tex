...
% --- ROW 1: Detection (1-Class) and Detection (2-Class) ---
\begin{figure}[htbp]
	\centering
...
% --- ROW 2: Detection (3-Class) and RoI Classification (2-Class) ---
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{0.49\textwidth}
		\centering
...
% --- ROW 3: RoI Classification (3-Class) and Segmentation ---
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[b]{0.49\textwidth}
		\centering
...
\begin{figure}[htbp] 
	...
	\begin{tikzpicture} 
		\begin{axis}[ 
			...
			% ========= PERFORMANCE ZONES (more subtle and clear) ========= 
			...
			% ========= VALUE LABELS ON BARS (2 decimal places) ========= 
			% 1-Class values
...
\textbf{Observations:} Across our detection experiments, the impact of auxiliary tasks depends on the complexity of the primary task. For 1-Class Detection, adding auxiliary tasks reduces performance, with F1(Inf) drops from 6\% to 11\%. For 2-Class Detection, RoI Classification provides the largest gain (+30.38\%), followed by Segmentation (+24.60\%) and Localization (+23.80\%), while combinations reduce incremental improvements (+16.46\% to +0.67\%). For 3-Class Detection, every MTTL variant improves over the STL baseline, with F1(Inf) gains from +44\% to +103.39\%, and the strongest single-task improvements coming from RoI and Segmentation. \\

\noindent Our experiments results provide evidence for our central hypothesis that auxiliary tasks, while sometimes detrimental to highly specialized single-task models, become increasingly beneficial as the complexity of the primary task grows.The benefits provided by auxiliary tasks depends on the complexity of the primary detection problem. As the task becomes more nuanced, auxiliary supervision can provide meaningful guidance, though the choice and combination of tasks.\\
...
\begin{figure}[htbp]
	\centering
	% --- Row 1: 2-Class STL/MTTL Correlation & Residuals ---
	...
	\vspace{2mm}
	% --- Row 2: 3-Class STL/MTTL Correlation & Residuals ---
	\begin{subfigure}[b]{0.23\textwidth}
...
\noindent The quantitative results are presented in \autoref{tab:parasitemia_results}. Single-class detection models are excluded from this analysis (indicated by dashes) as they lack the class distinction necessary for parasitemia estimation. Visual assessment is provided through correlation and residual plots in \autoref{fig:parasitemia_plots_all}, enabling direct evaluation of prediction accuracy and error distribution. Lower $\text{MAE}$ and higher Pearson's $r$ indicate superior estimation performance.\\

\noindent The results demonstrate substantial clinical advantages of MTTL models for parasitemia quantification. For 2-class detection, MTTL achieves $\text{MAE} = 0.92\%$ versus STL's $2.26\%$ (a $59\%$ reduction), while improving correlation from $r = 0.90$ to $r = 0.98$. For 3-class detection, MTTL attains $\text{MAE} = 1.08\%$ compared to STL's $3.30\%$ (a $67\%$ reduction), with correlation increasing from $r = 0.81$ to $r = 0.97$. These improvements confirm that multi-task learning improves not only detection robustness but also precise infection load quantification.\\

\noindent The correlation plots reveal that MTTL predictions align a little more tightly with ground-truth parasitemia across the full range of infection levels. Correspondingly, the residual plots show that MTTL errors are smaller, more uniformly distributed, and exhibit less systematic bias than STL models. This combination of improved accuracy, reduced variance, and minimized bias highlights the practical reliability of MTTL for slide-level clinical assessments.